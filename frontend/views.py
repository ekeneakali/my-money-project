...
def home(request):
    ...
    return render(request, 'frontend/index.html', {'page_obj':page_obj, 'latest':latest})
...
def shop(request):
    ...
    return render(request, 'frontend/shop.html', {'page_obj':page_obj, 'late':late})
...
def two(request, pst):
    ...
    return render(request, 'frontend/two.html', {'race':race, 'is_liked':is_liked, 'total_likes':race.total_likes, 'is_favourite':is_favourite, 'product':product})
...
def blog_two(request, pst):
    if not request.user.is_authenticated:
        return redirect('frontend:log_in')


    race = Post.objects.get(pk=pst)
    recent = Post.objects.all()[:2]
    trending = Product.objects.all()[:2]
    comment = Comment.objects.filter(post__pk=pst)
    ...
    return render(request, 'frontend/blog-article.html', {'race':race, 'comment':comment, 'is_liked':is_liked, 'total_likes':race.total_likes, 'is_favourite':is_favourite, 'recent':recent, 'trending':trending})
...
def add_blog(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('frontend:add_blog')
...
def result(request):
    if request.method == 'GET':
        ...
        query_filter = Post.objects.filter(Q(title__icontains=search) | Q(description__icontains=search))
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        order = Order.objects.create(name=firstname, lastname=lastname, email=email,
        telephone=telephone, company=company, address_1=address_1, address_2=address_2, city=city,
        postcode=postcode, zone_id=zone_id, country_id=country_id)
        return redirect('frontend:order')
        order.save()
...
def delete_order(request, id):
    ...
    delete_ord = Order.objects.get(pk=id)
    delete_ord.delete()
...
def buy_now(request, pst):
    ...
    if request.method == 'POST':
        ...
        zone_id = request.POST.get('zone_id')
        country_id = request.POST.get('country_id')
        order = OrderBy.objects.create(name=firstname, lastname=lastname, email=email,
        telephone=telephone, company=company, address_1=address_1, address_2=address_2, city=city,
        postcode=postcode, zone_id=zone_id, country_id=country_id)
        order.save()
        return redirect('frontend:orderby')
        messages.success(request, 'payment made successfully!')
        print(f'{firstname} {lastname} {email} ')
    

    return render(request, 'frontend/buy-now.html', {'race':race})

def single_order(request, id):

    single_ord = Order.objects.get(pk=id)
    single_ord.delete()
    return redirect('frontend:order')
    messages.success(request, 'order deleted successfully!')   
...
def order(request):
    ...
    return render(request, 'frontend/order.html', {'order':order})

def orderby(request):

    order = OrderBy.objects.all()

    return render(request, 'frontend/orderby.html', {'order':order})
